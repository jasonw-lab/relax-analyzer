...
namespace analyzer.Core
{
    ...
    internal sealed class YearSheetService
    {
        ...
        private const int YearSheetMaxRow = 80;         // 80行目まで更新（81行目以降は関数式を保持）
        ...
        private int UpdateYearSheetMatrix(Excel.Worksheet yearSheet, Dictionary<string, decimal[]> monthlyData)
        {
            ...
            try
            {
                ...
                var lastRow = Math.Min(usedRange.Rows.Count, YearSheetMaxRow);
                ...
                for (var i = 1; i <= totalRows; i++)
                {
                    ...
                    // 「合計」行はスキップ（関数を上書きしないため）
                    if (string.Equals(type, "合計", StringComparison.OrdinalIgnoreCase))
                    {
                        continue;
                    }
